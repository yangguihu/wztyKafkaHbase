...
import cn.com.wiseweb.gather.{DomainUtils, HashUtil, KafkaHbaseUtils}
...
/**
  ...
  * 采集元搜索数据到hhbase
  */
object WeiboNew extends Serializable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    jobConf.set("mapreduce.output.fileoutputformat.outputdir", "/wzty/weibo")
    jobConf.setOutputFormat(classOf[TableOutputFormat])
    jobConf.set(TableOutputFormat.OUTPUT_TABLE, tableName)

    // Create context with 5 second batch interval
    val sparkConf = new SparkConf().setAppName("weibo")
...
//    ssc.checkpoint("hdfs://mycluster/wzty/webpagenew-ck")
  ...
  def parseSensor(record: String,bcWeiboKeyList :Broadcast[util.List[String]]): (String,JSONObject) = {
    ...
    try {
      ...
      var urlHash =HashUtil.xxHash(map.get("url"))

      var rowkey=urlHash.toString
      if(urlHash==0){
        rowkey=" "
      }
      //加一个group_id
