...
/**
  ...
  * 采集元搜索数据到hhbase
  */
object WeibouserNew extends Serializable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val group = "weibouser_hbase"
    //获取传入参数，便于调试
    var record="30"
    var interval="57"
    var tableName="weibouser"
    if(args.length!=0){
      record=args(0)
      interval=args(1)
      tableName=args(2)
    ...
    jobConf.set("mapreduce.output.fileoutputformat.outputdir", "/wzty/weibouser")
    jobConf.setOutputFormat(classOf[TableOutputFormat])
    jobConf.set(TableOutputFormat.OUTPUT_TABLE, tableName)

    // Create context with 5 second batch interval
    val sparkConf = new SparkConf().setAppName("weibouser")
