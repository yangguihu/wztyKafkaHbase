...
//          .setMaster("local[*]")
    ...
    var scan = new Scan()
    scan.setCaching(Integer.valueOf(cache))
...
//    hbaseConf.set(TableInputFormat.INPUT_TABLE, "newspaper_new")
    ...
    hBaseRDD.foreachPartition(it=>{
          //处理消息记录
          processPartition(it,bcCfSet,bcOutTable,bcSubmitcount)
        })
  }
  ...
  /**
    ...
    *   bcCfSet
    */
  def processPartition(partion: Iterator[(ImmutableBytesWritable, Result)],bcCfSet :Broadcast[util.HashSet[String]],
