...
/**
  ...
  * 采集元搜索数据到hhbase
  */
object WebpageHome_local extends Serializable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val group = "KHDZKWebpage_local"
    //获取传入参数，便于调试
    var record="5"
    var interval="8"
    var tableName="test"  //新闻
    var tableHome="hometest" //首页
    if(args.length!=0){
      record=args(0)
      interval=args(1)
      tableName=args(2)
    ...
    val sparkConf = new SparkConf().setAppName("KHDZKWebpage_local")
...
//        })
//          //.saveAsTextFile("file://home/hadoop/spark_job/errout/webpage.log")
//            .repartition(1).saveAsTextFile("file:///D:/wzty/errout/webpage.log")
...
//        map.put("errmsg",x.getMessage)
//        map.put("time",getDate())
//        map.put("topic","wiseweb_crawler_webpage")
        //返回错误信息
        (" ",null)
      }
    }
