...
import cn.com.wiseweb.gather.HashUtil
...
/**
  ...
  * 采集元搜索数据到hhbase
  */
object WeiboNew_local extends Serializable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("weibo_local")
...
//    ssc.checkpoint("hdfs://mycluster/wzty/webpagenew-ck")
  ...
  def parseSensor(record: String,bcWeiboKeyList :Broadcast[util.List[String]]): (String,JSONObject) = {
    ...
    try {
      //var map = JSON.parseObject(record,classOf[util.HashMap[String,String]])
      ...
      val urlHash =HashUtil.xxHash(map.get("url"))
      val strGather=getStrToTime(map.get("gathertime"))

      var rowkey= urlHash + "_" + strGather
      if (urlHash==0) rowkey=" "   //没有urlhash 直接回插入空 且只会插入一条
