...
/**
  ...
  * 采集元搜索数据到hhbase
  */
object NewsPaperNew_local extends Serializable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    jobConf.set("mapreduce.output.fileoutputformat.outputdir", "/wzty/paper")
    jobConf.setOutputFormat(classOf[TableOutputFormat])
    jobConf.set(TableOutputFormat.OUTPUT_TABLE, tableName)

    // Create context with 5 second batch interval
    val sparkConf = new SparkConf().setAppName("NewsPaperNew")
