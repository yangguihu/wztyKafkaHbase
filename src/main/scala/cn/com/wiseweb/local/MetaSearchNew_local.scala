...
import cn.com.wiseweb.gather.{DomainUtils, KafkaHbaseTools}
...
/**
  ...
  * 采集元搜索数据到hhbase
  */
object MetaSearchNew_local extends Serializable{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val group = "KHDZKMetaSerach_local"
    //获取传入参数，便于调试
    var record="30"
    var interval="8"
    var tableName="test2"
    ...
    val sparkConf = new SparkConf().setAppName("KHDZKMetaSerach_local")
...
//    ssc.checkpoint("hdfs://mycluster/wzty/webpagenew-ck")
    ...
    messages.foreachRDD(rdd=>{
      if (!rdd.isEmpty()) {
        rdd.map(record => {
          //解析，规整数据
          val map = parseSensor(record._2,bcWeiboKeyList,bcSengineMap,bcStypeMap)
          //添加到put
  ...
  def parseSensor(record: String,bcWeiboKeyList :Broadcast[util.List[String]]
    ...
    try {
      ...
      val urlHash:String=getUrlHash(map.getString("id"))
      val strGather=getStrToTime(map.getString("gathertime"))

      var rowkey= " "
      if (StringUtils.isNoneBlank(urlHash)) {//没有urlhash 直接回插入空 且只会插入一条
